...
const CalendarWidget = ({ onDaySelect, onNewPatient, onOpenAgenda }) => {
  ...
  const handleDayClick = (date) => {
    ...
    // Allow selecting any date (including past dates) to view medical records
        ...
        {calendarDays.map((date, index) => {
           ...
           return (
             <div
               ...
               className={`
                 relative p-1 text-center text-xs rounded transition-colors cursor-pointer
                 ${isCurrentMonthDay ? 'text-gray-900' : 'text-gray-400'}
                 ${isTodayDay && !isSelectedDay ? 'bg-blue-600 text-white hover:bg-blue-700' : ''}
                 ${isSelectedDay ? 'bg-green-600 text-white hover:bg-green-700' : ''}
                 ${isPastDay && isCurrentMonthDay && !isSelectedDay && !isTodayDay ? 'text-gray-600 bg-gray-50 hover:bg-gray-100' : ''}
                 ${!isPastDay && isCurrentMonthDay && !isSelectedDay && !isTodayDay ? 'hover:bg-blue-50' : ''}
               `}
               title={isCurrentMonthDay ? 'Click para ver citas de esta fecha' : ''}
             >
              <span className={`inline-block w-5 h-5 leading-5 rounded-full ${
                isTodayDay && !isSelectedDay ? 'bg-blue-600 text-white' :
